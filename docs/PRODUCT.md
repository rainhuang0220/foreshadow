...
Install (`uv tool install "git+https://github.com/rainhuang0220/foreshadow.git@v0.4.1"`) → token → `foreshadow init` → `foreshadow schedule install` (optional) or `foreshadow run` → `foreshadow board` → read why → **开始进入** → local prep → **等待你确认远程操作**.
